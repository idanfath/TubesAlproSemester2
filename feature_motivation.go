...
var motivations = []Motivation{
	{1, `Being resentful about your problems won’t solve them. What will solve them is focused action.
...
Your problems are yours to solve and doing so will improve life. Eagerly seize that opportunity and run with it`, "Ralph Marston"},
	{2, `Relish every experience. Put value into every moment.
...
Understand how this can be the best day yet. Give all you have to make it so.`, "Ralph Marston"},
}
...
func getDailyMotivation() Motivation {
	var id = randDaily(0, len(motivations)-1)
	return motivations[id]
}

func appDynamicGetDailyMotivation() []string {
	return []string{toString(getDailyMotivation().quote), "- " + toString(getDailyMotivation().author)}
}

func appDynamicGetRandomMotivation() []string {
	var motivation = getRandomMotivation()
	for motivation.id == Temp.lastMotivationID {
		motivation = getRandomMotivation()
	}
	Temp.lastMotivationID = motivation.id
	return []string{toString(motivation.quote), "- " + toString(motivation.author)}
}
